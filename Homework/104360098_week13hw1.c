...
#include <stdbool.h>
#include <math.h>
/*Computer Programming 2016 week 13 hw 1, Coded by Leung Ho Kwan 104360098*/
...
void initialbool(bool *overlap, int amount) {
    int i;
    for (i = 0; i < amount; i++) {
        overlap[i] = false;
    }
}
void countoverlap(bool *overlap, int amount, int *overlapcounter) {
    int i;
    for (i = 0; i < amount; i++) {
        if (overlap[i] == true) {
...
int main(int argc, char **argv) {
    int amount, i, j, intersect_counter = 0, overlapcounter = 0;
    ...
    overlap = (bool*)malloc(sizeof(bool) * amount); //allocate space for overlap bool
    initialbool(overlap, amount);   //set all overlap bool as false
    ...
    countoverlap(overlap, amount, &overlapcounter);
    printf("%d\n", overlapcounter);
    return 0;
}
